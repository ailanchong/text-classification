...
class TextRNN:
    def __init__(self,num_classes, learning_rate, batch_size, decay_steps, decay_rate,sequence_length,
        ...
        #self.predictions = tf.cast(tf.greater(self.logits, 0.5, name="predictions"),tf.float32)
        if not is_training:
            return
        self.loss_val, self.true_loss = self.loss() #-->self.loss_nce()
        self.train_op = self.train()
        #self.predictions = tf.argmax(self.logits, axis=1, name="predictions")  # shape:[None,]
        self.predictions = tf.greater(self.logits, 0.5, name="predictions")
