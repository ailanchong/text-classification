...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        ...
        if "train_history" in os.listdir("./") :
            with open("./train_histroy", 'rb') as file_in:
        ...
        for epoch in range(curr_epoch,FLAGS.num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
                ...
                if counter %10==0:
                    print("Epoch %d\tBatch %d\tTrain Loss:%.3f\tTrain TrueLoss:%.3f\tTrain Accuracy:%.3f" %(epoch,counter,curr_loss, curr_tureloss, curr_acc)) #tTrain Accuracy:%.3f---》acc/float(counter)
