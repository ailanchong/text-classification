...
from clean import clean, clean_twice
np.random.seed(2018)
...
list_sentences_train = train["comment_text"].fillna("_na_").apply(lambda x:clean_twice(x,swear_words, corr_map, knowset, keyset, commonset, in2out_map,vocab_in))
list_sentences_test = test["comment_text"].fillna("_na_").apply(lambda x:clean_twice(x,swear_words, corr_map, knowset, keyset, commonset, in2out_map,vocab_in))
train.to_csv('../data/twice/clean_train.txt',index=False)
test.to_csv("../data/twice/clean_test.txt", index=False)  
train_y = train[list_classes].values
#test_y = test[list_classes].values
corpus = list(list_sentences_test) + list(list_sentences_train) 
get_len_string(corpus)
with open("../data/twice/clean_corpus",'w') as file_out:
    for line in corpus:
        file_out.write(line + "\n")
