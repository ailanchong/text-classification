...
class fastTextB:
    def __init__(self, label_size, learning_rate, batch_size, decay_steps, decay_rate,num_sampled,sentence_len,vocab_size,embed_size,is_training,max_label_per_example=5):
        ...
        #6.calculate loss
        self.loss_val, self.true_loss = self.loss()
    ...
    def loss(self,l2_lambda=0.0001):  #0.0001
        ...
        ''' 
        self.labels_l1999 = tf.cast(self.labels_l1999, tf.float32)
        #labels_multi_hot = self.labels_l1999 #[batch_size,label_size]
        #labels_multi_hot = tf.cast(labels_multi_hot,tf.float32)
        ...
        loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=self.labels_l1999,logits=self.logits) #labels:[batch_size,label_size];logits:[batch, label_size]
