...
import numpy as np
from model import fastTextB as fastText
...
tf.app.flags.DEFINE_integer("label_size",6,"number of label")
...
tf.app.flags.DEFINE_integer("sentence_len",800,"max sentence length")
tf.app.flags.DEFINE_integer("embed_size",300,"embedding size") #100
tf.app.flags.DEFINE_boolean("is_training",True,"is traning.true:tranining,false:testing/inference")
tf.app.flags.DEFINE_integer("num_epochs",8,"embedding size")
    ...
    with tf.Session(config=config) as sess:
        ...
        if os.path.exists(FLAGS.ckpt_dir+"checkpoint"):
            ...
            if FLAGS.use_embedding: #load pre-trained word embedding
                assign_pretrained_word_embedding(sess, embed_matrix, fast_text)
        ...
        for epoch in range(curr_epoch,FLAGS.num_epochs):#range(start,stop,step_size)
            loss, trueloss, counter = 0.0, 0.0, 0
            #for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
            for i in range((number_of_training_data / batch_size) + 1):
                ...
                curr_loss, curr_trueloss, _,step_num=sess.run([fast_text.loss_val, fast_text.true_loss, fast_text.train_op,fast_text.global_step],feed_dict={fast_text.sentence:trainX[start:end],fast_text.labels_l1999:trainY[start:end]}) #fast_text.labels_l1999:trainY1999[start:end]
                loss, trueloss, counter=loss+curr_loss, trueloss+curr_trueloss, counter+1    #acc+curr_acc,
                step_list.append(step_num)
                loss_list.append(curr_trueloss)
                if counter %10==0:
                    print("Epoch %d\tBatch %d\tTrain Loss:%.3f\tTrue Loss:%.3f" %(epoch,counter,loss/float(counter), trueloss/float(counter))) #\tTrain Accuracy:%.3f--->,acc/float(counter)
                
                if step_num % 50 == 0:
                    test_step_list.append(step_num)
                    eval_loss, eval_trueloss =do_eval(sess,fast_text,testX,testY,batch_size)
                    print("Epoch %d Validation Loss:%.3f\tValidation trueLoss:%.3f" % (epoch,eval_loss,eval_trueloss))
...
def do_eval(sess,fast_text,evalX,evalY,batch_size):
    number_examples=len(evalX)
    eval_loss,trueloss,eval_counter=0.0, 0.0, 0
    for start,end in zip(range(0,number_examples,batch_size),range(batch_size,number_examples,batch_size)):
        curr_eval_loss, curr_tureloss, logits= sess.run([fast_text.loss_val, fast_text.true_loss, fast_text.logits],#curr_eval_acc--->textCNN.accuracy
                                          feed_dict={fast_text.sentence: evalX[start:end],fast_text.labels_l1999: evalY[start:end]
                                              })
        #label_list_top5 = get_label_using_logits(logits_[0], vocabulary_index2word_label)
        #curr_eval_acc=calculate_accuracy(list(label_list_top5), evalY[start:end][0],eval_counter)
        eval_loss, trueloss,eval_counter=eval_loss+curr_eval_loss, trueloss+curr_tureloss,eval_counter+1
    return eval_loss/float(eval_counter), trueloss/float(eval_counter)
...
'''
def assign_pretrained_word_embedding(sess, embed_matrix, fast_text):
    word_embedding = tf.constant(embed_matrix, dtype=tf.float32)
    t_assign_embedding = tf.assign(fast_text.Embedding, word_embedding)
    sess.run(t_assign_embedding)
    print("using pre-trained word emebedding.ended...")
